...
A(para("p1a", run(
  ...
  "objective. Because this is a temporal-representation gap rather than a model-capacity "
  "limitation, the planned remedy is a graded comparison of time-aware detection — "
  "change-point monitoring (CUSUM, consistent with the statistical-process-control basis of "
  "the baseline) and longitudinal trend features, evaluated against sequence models "
  "(LSTM / Transformer autoencoders) — to establish how much temporal modelling low-and-slow "
  "detection actually requires; an initial Transformer prototype did not yet improve Scenario "
  "2, plausibly constrained by only 70 insider entities, so the contribution is the comparison "
  "itself. The primary detection-quality direction, however, is improving multi-cloud coverage "
  "through per-source feature engineering — for example incorporating Azure's native risk "
  "signals — since the cloud autoencoder was trained on AWS CloudTrail. The system does not yet detect model drift as legitimate "
